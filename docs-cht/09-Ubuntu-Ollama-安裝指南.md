...
| 「pip install 被擋了」 | `openshell policy set --policy nemoclaw-blueprint/policies/presets/pypi.yaml my-assistant` | 0 |
...
```bash
# 1. 確認 Docker 與 Ollama 已啟動
sudo systemctl start docker
sudo systemctl start ollama  # 若使用本地推論

# 2. 重啟已停止的閘道容器（不會銷毀沙箱）
docker start openshell-cluster-nemoclaw

# 3. 等待閘道內的 k3s 叢集完全就緒（關鍵步驟！）
...
# 4. 檢查沙箱 pod 是否真正存在
#    重新開機後 k3s pod 可能未被恢復，sandbox list 顯示 Ready 但
#    實際 connect 會出現 exit 255 或 "sandbox not found"。
if openshell sandbox connect my-assistant -- echo ok 2>/dev/null; then
  echo "✓ 沙箱可連線"
  nemoclaw my-assistant connect
else
  echo "✗ 沙箱 pod 已遺失，需要重建（等級 1，閘道保留）"
  echo "  .openclaw-data/ 中的備份會在重建時自動注入"
  cd ~/NemoClaw  # 或你的 NemoClaw 原始碼目錄
  NEMOCLAW_NON_INTERACTIVE=1 \
  NEMOCLAW_SANDBOX_NAME=my-assistant \
  NEMOCLAW_PROVIDER=ollama \
...
> **常見錯誤：`tls handshake eof`** — 閘道容器已啟動但 k3s 尚未就緒。等待 `openshell status` 顯示 `Connected` 後再繼續（通常 15-30 秒）。

> **常見錯誤：`exit 255` 或 `sandbox not found`** — 閘道就緒但沙箱 k3s pod 在重新開機後未被恢復。`openshell sandbox list` 可能仍顯示 Ready（過期紀錄），但實際連線會失敗。此時需要等級 1 重建沙箱。若已透過 `pull-openclaw-data.sh` 備份，重建時 `.openclaw-data/` 會自動注入，Agent 的記憶（MEMORY.md、SOUL.md 等）不會遺失。

> **閘道容器命名規則：** `openshell-cluster-<gateway-name>`，本指南中為 `openshell-cluster-nemoclaw`。

> **若等待 60 秒仍未就緒：** 檢查 `docker logs openshell-cluster-nemoclaw --tail 50`。若持續錯誤，需等級 3 重建（見「變更設定：增量更新 vs 完整重建」）。
...
NEMOCLAW_DIR="${NEMOCLAW_DIR:-/home/${SUDO_USER:-$USER}/NemoClaw}"
...
# 檢查沙箱 pod 是否真正存在
# 重新開機後 k3s pod 可能未被恢復，需要重建沙箱
if "$OPENSHELL_BIN" sandbox connect "$SANDBOX_NAME" -- echo ok >/dev/null 2>&1; then
  echo "Sandbox '$SANDBOX_NAME' is alive"
else
  echo "Sandbox pod lost after reboot — rebuilding (gateway preserved)..."
  if [ -d "$NEMOCLAW_DIR" ]; then
    ...
    NEMOCLAW_POLICY_MODE=suggested \
    NEMOCLAW_RECREATE_SANDBOX=1 \
    nemoclaw onboard --non-interactive 2>&1 || true
  else
    echo "WARNING: $NEMOCLAW_DIR not found, cannot auto-rebuild sandbox"
  fi
fi

echo "NemoClaw gateway started, sandbox '$SANDBOX_NAME' ready"
SCRIPT
...
[Unit]
Description=NemoClaw OpenShell Gateway + Sandbox Auto-Recovery
...
Type=oneshot
RemainAfterExit=yes
# 沙箱重建可能需要 10-15 分鐘，預設 90 秒會 timeout
TimeoutStartSec=900
...
Environment=NEMOCLAW_DIR=/home/<your-username>/NemoClaw
...
docker start "$GATEWAY_CONTAINER" 2>&1 || true
for i in $(seq 1 30); do "$OPENSHELL_BIN" status 2>&1 | grep -q "Connected" && break; sleep 2; done
if "$OPENSHELL_BIN" sandbox connect "$SANDBOX_NAME" -- echo ok >/dev/null 2>&1; then
  echo "Sandbox '$SANDBOX_NAME' is alive"
else
  echo "Sandbox pod lost — rebuilding..."
  cd "$NEMOCLAW_DIR" 2>/dev/null && \
  NEMOCLAW_NON_INTERACTIVE=1 NEMOCLAW_SANDBOX_NAME="$SANDBOX_NAME" \
  NEMOCLAW_PROVIDER=ollama NEMOCLAW_MODEL=glm-5:cloud \
  NEMOCLAW_POLICY_MODE=suggested NEMOCLAW_RECREATE_SANDBOX=1 \
  nemoclaw onboard --non-interactive 2>&1 || echo "WARNING: auto-rebuild failed"
fi
SCRIPT
sudo chmod +x /usr/local/bin/nemoclaw-gateway-start.sh
...
Environment=SANDBOX_NAME=my-assistant
Environment=NEMOCLAW_DIR=/home/${USERNAME}/NemoClaw
ExecStart=/usr/local/bin/nemoclaw-gateway-start.sh
User=${USERNAME}
Group=docker
...
**解決：** 參閱上方「重新開機後恢復」章節的完整手動恢復流程。可能有兩種情況：

**情況 A：閘道未就緒（`tls handshake eof`）**

`docker start` 後 k3s 需要 15-30 秒初始化。等待 `openshell status` 顯示 `Connected` 後再繼續。

**情況 B：沙箱 pod 遺失（`exit 255` 或 `sandbox not found`）**

閘道已就緒且 `openshell sandbox list` 顯示 Ready，但 `nemoclaw my-assistant connect` 失敗。這是因為 k3s pod 在重新開機後未被恢復（`sandbox list` 的 Ready 是過期紀錄）。

驗證方式：

```bash
# 若閘道已 Connected，嘗試連線
openshell sandbox connect my-assistant -- echo ok
# 若出現 exit 255 → pod 已遺失，需等級 1 重建沙箱
```

此時需要重建沙箱（閘道保留，~5-10 分鐘）。若已透過 `pull-openclaw-data.sh` 備份 `.openclaw-data/`，重建時會自動注入，**Agent 的記憶不會遺失**。
...
> **建議：** 設定 systemd 自動啟動服務（見「自動啟動」章節），腳本已包含沙箱 pod 遺失時的自動重建邏輯。

> **警告：** 請勿使用 `openshell gateway start --name nemoclaw` 恢復，該指令會提示銷毀並重建閘道，導致所有沙箱遺失。
...
# 套用 npm 預設集
openshell policy set --policy nemoclaw-blueprint/policies/presets/npm.yaml my-assistant
```
...
# 動態套用（臨時）
openshell policy set --policy nemoclaw-blueprint/policies/presets/my-api.yaml my-assistant
...
# 動態套用策略
openshell policy set --policy <policy-file> my-assistant
...
```bash
openshell policy set --policy nemoclaw-blueprint/policies/presets/ollama-web.yaml my-assistant
```
...
# Host 端執行（非沙箱內）
cd ~/nemo-claw
openshell policy set --policy nemoclaw-blueprint/policies/presets/ollama-local.yaml my-assistant
```
...
> **養成習慣：離開沙箱前先備份。** 重新開機後沙箱 k3s pod 可能遺失，需重建沙箱。若未備份，沙箱內的記憶（MEMORY.md、SOUL.md 等）和新建檔案會遺失。
>
> ```bash
> # 每次結束工作時執行（Host 端，非沙箱內）
> ./scripts/pull-openclaw-data.sh my-assistant
> ```
