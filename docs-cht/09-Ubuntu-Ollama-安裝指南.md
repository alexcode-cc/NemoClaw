...
| 新增/修改網路策略 | `nemoclaw <name> policy-add` | ~1 分鐘 | 否 | 否 |
...
| 「pip install 被擋了」 | `nemoclaw my-assistant policy-add
# 在互動選單中選擇 pypi` | 0 |
...
| 套用 ollama-local 策略（僅方式 B） | Host 端 | `nemoclaw <name> policy-add` | 0 |
...
| 查詢 Ollama 模型資訊 | Host 端 | `nemoclaw <name> policy-add` → ollama-web | 0 |
