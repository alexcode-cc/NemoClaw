...
**症狀：** `nemoclaw my-assistant connect` 出現 `transport error` → `Connection refused (os error 111)`

**原因：** OpenShell 閘道的 Docker 容器在重新開機後處於停止狀態。
...
> **警告：** 請勿使用 `openshell gateway start --name nemoclaw`，該指令會提示銷毀並重建閘道，導致所有沙箱遺失。若不慎已銷毀，需加上 `NEMOCLAW_RECREATE_SANDBOX=1` 重新執行 onboard。

若需自動啟動，請參閱上方「重新開機後恢復」章節設定 systemd 服務。
